...
async createLogsForUpdate(existingEvent, updateData, profileNames) {
  ...
  if (updateData.profiles) {
    ...
    if (JSON.stringify(oldIds) !== JSON.stringify(newIds)) {
      ...
      const newProfileNames = profileNames && profileNames.length > 0
        ? profileNames.join(", ")
        : "updated profiles";
  ...
  if (updateData.startDate) {
    ...
    if (oldDate !== newDate) {
      const oldFormatted = this.formatDateTime(existingEvent.startDate, existingEvent.timezone);
      const newFormatted = this.formatDateTime(new Date(updateData.startDate), updateData.timezone || existingEvent.timezone);
  ...
  if (updateData.endDate) {
    ...
    if (oldDate !== newDate) {
      const oldFormatted = this.formatDateTime(existingEvent.endDate, existingEvent.timezone);
      const newFormatted = this.formatDateTime(new Date(updateData.endDate), updateData.timezone || existingEvent.timezone);
